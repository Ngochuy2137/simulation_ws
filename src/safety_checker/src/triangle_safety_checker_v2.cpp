...
#define obs_count_threshold 2
#define obs_cost_value_threshold 100    // If cost value of a cell >= obs_cost_value_threshold, it is occupancied
  ...
  void check_safety()
  {
    ...
    for (int i=0; i<num_safe_point; i++)
    {
      ...
      if(a)
      {
        ...
        // cout << "=============  HELLO 1 ====================" << endl;  
        // cout << costmap.data[11] << endl;
        // cout << "---" << endl;
        // cout << costmap.data[index] << endl;
        unsigned int cost = costmap.data[index];
        // cout << "=============  HELLO 2 ====================" << endl;

        safe_triangle[i].cost = cost;
        if (cost >=obs_cost_value_threshold)  
        {
          obs_count ++;
        }
      }
...
void get_costmap_info_cb(const nav_msgs::OccupancyGrid::ConstPtr& msg)
{
  ...
  map_info.set_info( costmap_info.info.origin.position.x,
            ...
            0.05, // costmap_info.info.resolution,
            200,  // costmap_info.info.width,
            200); // costmap_info.info.height);
...
void update_costmap_cb(const nav_msgs::OccupancyGrid::ConstPtr& msg)
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_costmap = node.subscribe(COSTMAP_INFO_TOPIC, 1, update_costmap_cb);
  ...
  ros::Publisher safety_checker_pub = node.advertise<std_msgs::Bool>("e_stop", 10);
  ...
  while(node.ok())
  {
    ...
    // safety_checker_pub.publish(e_stop_msg);
    safety_checker_pub.publish(e_stop_msg);
